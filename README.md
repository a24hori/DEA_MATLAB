...
<li> <b>DEA(D,"入力の数","出力の数")</b>の形で関数の引数を入力します。"入力の数"+"出力の数"が項目の総数になっていない場合はエラーがでます。データ行列を転置するかどうかは<b>「行にDMU、列に項目が並ぶ形」</b>でデータ行列を作成した場合はデータ行列の部分は<b> D.' </b>入力としてください。先の例の場合、データ行列は転置して入力する必要がありますので、<b>DEA(D.',2,2)</b>のように入力します。</li>
